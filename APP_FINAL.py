...
.nav-logo .dot { color: var(--accent); font-size: 0.6rem; vertical-align: middle; }
...
.hero-line:nth-child(3) { transition-delay: 0.25s; }
.hero-line:nth-child(4) { transition-delay: 0.35s; }
...
.metric-val.animated { animation: flashIn 0.5s cubic-bezier(0.22,1,0.36,1) both; }
</style>
...
<nav class="nav-bar" id="main-nav">
  <div class="nav-logo">✈ RUL <span class="dot">●</span> PREDICTOR</div>
  <div class="nav-links">
    <a class="nav-link" data-section="hero"      href="#hero">Overview</a>
    <a class="nav-link" data-section="rul"       href="#rul">RUL</a>
    <a class="nav-link" data-section="dataset"   href="#dataset">Dataset</a>
    <a class="nav-link" data-section="pipeline"  href="#pipeline">Pipeline</a>
    <a class="nav-link" data-section="models"    href="#models">Models</a>
    <a class="nav-link" data-section="results"   href="#results">Results</a>
    <a class="nav-link" data-section="predictor" href="#predictor">Live Predictor</a>
    <a class="nav-link" data-section="takeaways" href="#takeaways">Takeaways</a>
  </div>
</nav>
...
(function() {
  function init() {
    ...
    var sections = ['hero','rul','dataset','pipeline','models','results','predictor','takeaways'];
    function updateActive() {
      ...
      sections.forEach(function(id) {
        var el = document.getElementById(id);
        if (el && el.getBoundingClientRect().top <= navH + 10) current = id;
      });
    ...
    function revealOnScroll() {
      ...
      revealEls.forEach(function(el) {
        if (el.getBoundingClientRect().top < winH * 0.88) el.classList.add('visible');
      });
      heroLines.forEach(function(el) {
        if (el.getBoundingClientRect().top < winH * 0.95) el.classList.add('visible');
      });
      metricVals.forEach(function(el) {
        if (el.getBoundingClientRect().top < winH * 0.9 && !el.classList.contains('animated')) {
...
<div class="section section-dark">
  <div class="sec-label reveal">NASA CMAPSS FD001 &nbsp;·&nbsp; Deep Learning &nbsp;·&nbsp; Time Series</div>
  <h1 class="hero-title">
    <span class="hero-line">PREDICTING</span>
    <span class="hero-line">ENGINE <em>DEATH</em></span>
    <span class="hero-line">BEFORE IT</span>
    <span class="hero-line">HAPPENS.</span>
  </h1>
  <p class="reveal reveal-delay-2" style="font-size:1rem; color:#555; max-width:480px; line-height:1.7; margin-bottom:2rem;">
  ...
  <div class="reveal reveal-delay-3" style="display:flex; gap:0.5rem; flex-wrap:wrap; margin-bottom:2.5rem;">
...
st.markdown("""
<div class="metric-row reveal reveal-delay-4">
...
<div class="section section-alt">
  ...
  <h2 class="section-title reveal reveal-delay-1">WHAT EVEN IS RUL?</h2>
  <p class="section-sub reveal reveal-delay-2">The clock every machine is running against — you just can't see it.</p>
""", unsafe_allow_html=True)
...
st.markdown("""
<div class="section section-dark">
  <div class="sec-label reveal">02 &nbsp;/&nbsp; Dataset</div>
  <h2 class="section-title reveal reveal-delay-1">NASA CMAPSS FD001</h2>
  <p class="section-sub reveal reveal-delay-2">100 training engines. 100 test engines. 26 raw columns. One mission: predict failure.</p>
""", unsafe_allow_html=True)
...
st.markdown("""
<div class="section section-alt">
  <div class="sec-label reveal">03 &nbsp;/&nbsp; Pipeline</div>
  <h2 class="section-title reveal reveal-delay-1">HOW THE MAGIC WORKS</h2>
  <p class="section-sub reveal reveal-delay-2">Raw CSVs → cleaned sequences → trained model → RUL prediction. Every step, explained.</p>
""", unsafe_allow_html=True)
...
for i, (num, title, detail) in enumerate(steps):
    ...
    delay_cls = f"reveal-delay-{(i % 4) + 1}"
    col.markdown(f"""
    <div class="pipe-step reveal {delay_cls}">
...
st.markdown("""
<div class="section section-dark">
  <div class="sec-label reveal">04 &nbsp;/&nbsp; Models</div>
  <h2 class="section-title reveal reveal-delay-1">7 MODELS. ONE WINNER.</h2>
  <p class="section-sub reveal reveal-delay-2">From a simple linear baseline to stacked bidirectional LSTMs — here's how they all stacked up.</p>
""", unsafe_allow_html=True)
...
st.markdown("""
<div class="section section-alt">
  <div class="sec-label reveal">05 &nbsp;/&nbsp; Results</div>
  <h2 class="section-title reveal reveal-delay-1">WHAT THE MODEL LEARNED</h2>
  <p class="section-sub reveal reveal-delay-2">Final model: Stacked LSTM · RMSE 25.57 · MAE 20.39 · R² 0.51 on 100 test engines.</p>
""", unsafe_allow_html=True)
...
st.markdown("""
<div class="section section-dark">
  <div class="sec-label reveal">06 &nbsp;/&nbsp; Live Predictor</div>
  <h2 class="section-title reveal reveal-delay-1">TRY IT LIVE.</h2>
  <p class="section-sub reveal reveal-delay-2">Load your trained model and scaler to get a real-time RUL prediction from sensor values.</p>
""", unsafe_allow_html=True)
...
st.markdown("""
<div class="section section-alt">
  <div class="sec-label reveal">07 &nbsp;/&nbsp; Takeaways</div>
  <h2 class="section-title reveal reveal-delay-1">WHAT I LEARNED.</h2>
  <p class="section-sub reveal reveal-delay-2">Key insights from building this end-to-end predictive maintenance system.</p>
""", unsafe_allow_html=True)
...
with col1:
    ...
    for i, (title, detail) in enumerate(takeaways):
        delay_cls = f"reveal-delay-{i+1}"
        st.markdown(f"""
        <div class="card-sm reveal {delay_cls}">
